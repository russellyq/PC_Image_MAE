...
from .model_mae import mae_vit_base_patch16, mae_vit_large_patch16, mae_vit_huge_patch14, MaskedAutoencoderViT
...
class MAE(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        _, _, H_raw, W_raw = raw_images.size() # (256, 1024)
...
class MAE_V2(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.range_encoder = mae_vit_large_patch16(in_chans=5, img_with_size=self.range_img_size, out_chans=5, with_patch_2d=(4, 16))
    ...
    def forward(self, batch_dict):
        ...
        img_token_2_range[:, :, 24:40, :] = torch.nn.functional.interpolate(img_mask_full_tokens_cls_unpatch.permute(0, 3, 1, 2).contiguous(), size=(h_img, h_img), mode='bilinear').permute(0, 2, 3, 1).contiguous().detach()
        img_token_2_range.requires_grad=True

        range_token_2_img = torch.nn.functional.interpolate(range_mask_full_tokens_cls_unpatch[:, :, 24:40, :].permute(0, 3, 1, 2).contiguous(), size=(h_img, w_img), mode='bilinear').permute(0, 2, 3, 1).contiguous()
