...
class SemanticKitti(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.dataset_cfg = dataset_cfg
        with open(dataset_cfg['dataset_params']['label_mapping'], 'r') as stream:
        ...
        for i_folder in split:
            self.im_idx += absoluteFilePaths('/'.join([str(self.root_path), str(i_folder).zfill(2), 'velodyne']), num_vote)
            calib_path = os.path.join(str(self.root_path), str(i_folder).zfill(2), "calib.txt")
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['laser_range', 'laser_ori_xyz',
                             'laser_ori_r', 'laser_idx',
                             'laser_mask', 'laser_range_in']:
                    values = []
                    for item in val:
                        values.append(item)
                    ret[key] = np.stack(values, 0)
                
                elif key in ['laser_y', 'laser_x', 'laser_points']:
                    coor_pad = []
                    for i, coor in enumerate(val):
                        if key in ['laser_y', 'laser_x']:
                            coor = coor.reshape((-1, 1))
                        elif key in ['laser_points']:
                            coor = coor.reshape((-1, 4))
                        # print('key, value', key, coor.shape)
                        coors = np.concatenate([i * np.ones((coor.shape[0], 1)),coor], -1)
                        coor_pad.append(coors)
                    # print('key, value', key, np.concatenate(coor_pad, axis=0).shape)
    ...
    def __len__(self):
        ...
        if self._merge_all_iters_to_one_epoch:
            return len(self.im_idx) * self.total_epochs
    ...
    def __getitem__(self, index):
        ...
        keep_idx_img_pts = self.select_points_in_frustum(img_points, 0, 0, image.shape[0], image.shape[1])
        ...
        top = image.shape[0] - self.bottom_crop[1]
        ...
        # top = self.dataset_cfg['bottom_crop']['top']
        # bottom = self.dataset_cfg['bottom_crop']['bottom']
        # # update image points
        # keep_idx = points_img[:, 0] >= top
        # keep_idx = np.logical_and(keep_idx, points_img[:, 0] < bottom)
        # keep_idx = np.logical_and(keep_idx, points_img[:, 1] >= left)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        ROOT_DIR = Path('/home/yanqiao/Documents/kitti')
        create_kitti_infos(
