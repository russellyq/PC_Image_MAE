...
class voxelization(nn.Module):
    def __init__(self, coors_range_xyz, spatial_shape, scale_list, sample_points=False):
...
class voxel_3d_generator(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.sample_points:
            ...
            features = torch_scatter.scatter_mean(pt_fea, data_dict['sample_points_scale_1']['coors_inv'], dim=0)
            data_dict['sample_points_sparse_tensor'] = spconv.SparseConvTensor(
                features=features,
                indices=data_dict['sample_points_scale_1']['coors'].int(),
                spatial_shape=np.int32(self.spatial_shape)[::-1].tolist(),
                batch_size=data_dict['batch_size']
            )

            data_dict['sample_points_coors'] = data_dict['sample_points_scale_1']['coors']
            data_dict['sample_points_coors_inv'] = data_dict['sample_points_scale_1']['coors_inv']
            data_dict['sample_points_full_coors'] = data_dict['sample_points_scale_1']['full_coors']
